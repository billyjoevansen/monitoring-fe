...
export default function SummarySortStrip({
  ...
  return (
    <div className="flex flex-col sm:flex-row items-stretch bg-white dark:bg-slate-900 border border-border rounded-xl overflow-hidden text-sm mb-6 shadow-sm">
      {/* Total Petani */}
      <button
        type="button"
        onClick={() => onSort(null)}
        className={`flex-1 px-4 py-3 text-center sm:border-r border-b sm:border-b-0 border-border/60 transition-all duration-200 cursor-pointer ${
          activeKey === null
            ? 'bg-blue-50 dark:bg-blue-950/40 shadow-inner'
            : 'hover:bg-muted/50'
        }`}
      >
      ...
      <button
        ...
        className={`flex-1 px-4 py-3 text-center sm:border-r border-b sm:border-b-0 border-border/60 transition-all duration-200 cursor-pointer ${
          activeKey === 'normal'
            ? 'bg-emerald-50 dark:bg-emerald-950/40 shadow-inner'
            : 'bg-emerald-50/30 dark:bg-emerald-950/10 hover:bg-emerald-100/60 dark:hover:bg-emerald-950/30'
        }`}
      >
      ...
      <button
        ...
        className={`flex-1 px-4 py-3 text-center transition-all duration-200 cursor-pointer ${
          activeKey === 'tidak_normal'
            ? 'bg-red-50 dark:bg-red-950/40 shadow-inner'
            : 'bg-red-50/30 dark:bg-red-950/10 hover:bg-red-100/60 dark:hover:bg-red-950/30'
        }`}
      >
