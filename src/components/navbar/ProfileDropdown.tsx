...
export function ProfileDropdown({
  ...
  return (
    <div ref={dropdownRef} className="relative hidden lg:block">
      <button
        onClick={onToggle}
        className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg hover:bg-gray-100/80 dark:hover:bg-slate-800/80 transition-colors"
      >
        ...
        <div className="text-left hidden xl:block">
          <p className="text-xs font-semibold text-gray-800 dark:text-white leading-tight truncate max-w-30">
            {user.nama}
          </p>
        ...
        <ChevronDown
          className={`w-3.5 h-3.5 text-gray-400 dark:text-slate-500 transition-transform ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
