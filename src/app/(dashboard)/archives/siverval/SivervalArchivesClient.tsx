...
const SIVERVAL_GROUPS: ColumnGroup[] = [
  {
    label: 'Identitas',
    columns: [
      { key: 'NO', label: 'No.' },
      ...
      { key: 'NO TRANSAKSI', label: 'No Transaksi' },
      { key: 'KODE KIOS', label: 'Kode Kios' },
      { key: 'NAMA KIOS', label: 'Nama Kios' },
      { key: 'NIK', label: 'NIK' },
  ...
  },
  {
    ...
    columns: [
      ...
      { key: 'SP36', label: 'SP36' },
      { key: 'ZA', label: 'ZA' },
      { key: 'NPK FORMULA', label: 'NPK Formula' },
      { key: 'ORGANIK', label: 'Organik' },
      { key: 'ORGANIK CAIR', label: 'Organik Cair' },
    ],
  },
  {
    label: 'Status',
    columns: [
      { key: 'TGL TEBUS', label: 'Tgl Tebus' },
      { key: 'TGL INPUT', label: 'Tgl Input' },
      { key: 'STATUS', label: 'Status' },
    ],
  },
];

interface SivervalArchivesClientProps {
  userId: string;
  canAccess: boolean;
  userRole: Role;
...
export default function SivervalArchivesClient({
  ...
  const filteredDocs = sivervalDocs.filter((d) => {
  ...
  const handleUse = useCallback(
    async (doc: (typeof sivervalDocs)[0]) => {
      router.push(`/reconcile?doc_id=${doc.id}&type=siverval`);
    },
    [router],
  );
  ...
  return (
    <div>
      ...
      ) : sivervalDocs.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-gray-300 p-12 text-center">
          <FileUp className="w-12 h-12 text-gray-300 dark:text-gray-600 mx-auto mb-3" />
          <p className="text-sm font-semibold text-foreground">Belum ada dokumen Si-Verval</p>
